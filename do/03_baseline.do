...
* Analysis: Balance table for baseline characteristics
...
*------------------------------------------------------------------------------*
*------------------------------------------------------------------------------*
* 	Baseline school level characteristics
*------------------------------------------------------------------------------*
*------------------------------------------------------------------------------*
...
reg exp_1 parent_prim pb21 pb06 teachedu_col teachedu_uni frate_tot_both rrate_tot_both drate_tot_both if year == 2007
...
reg exp_2 parent_prim pb21 pb06 teachedu_col teachedu_uni frate_tot_both rrate_tot_both drate_tot_both if year == 2009
testparm parent_prim pb21 teachedu_col teachedu_uni frate_tot_both rrate_tot_both drate_tot_both, equal 
local f2 = r(p)
local f2: di %6.3f `f2'
...
texdoc init balance1.tex, replace force
...
tex Number of schools & \multicolumn{2}{c}{125} & \multicolumn{2}{c}{125} & & & \\
...
texdoc init balance2.tex, replace force
...
tex Number of schools & \multicolumn{2}{c}{100} & \multicolumn{2}{c}{80} & & & \\
...
reg `var' i.exp_1 if year==2007, robust cluster(cct)
  global diff1_`var': di %-6.3fc _b[1.exp_1]
  global se1_`var': di %-4.3fc _se[1.exp_1]
...
reg `var' i.exp_2 if year==2009, robust cluster(cct)
  global diff2_`var': di %-6.3fc _b[1.exp_2]
  global se2_`var' : di %-4.3fc _se[1.exp_2]
...
* F-test for experiment 1
reg exp_1 ap03 female zwealth grade3 grade4 grade5 punt_esp punt_mat if year == 2007, robust cluster(cct)
testparm female zwealth grade3 grade4 grade5 punt_esp punt_mat, equal 
local f1 = r(p)
local f1: di %6.3f `f1'

* F-test for experiment 2
reg exp_2 ap03 female zwealth grade3 grade4 grade5 punt_esp punt_mat if year == 2009, robust cluster(cct)
testparm female zwealth grade3 grade4 grade5 punt_esp punt_mat, equal 
local f2 = r(p)
local f2: di %6.3f `f2'

* Get N
count if year == 2007 & exp_1 == 0
	local N1_c = r(N)
	
count if year == 2007 & exp_1 == 1
	local N1_t = r(N)

count if year == 2009 & exp_2 == 0
	local N2_c = r(N)
	
count if year == 2009 & exp_2 == 1
	local N2_t = r(N)	
...
texdoc init balance1.tex, append force
...
texdoc init balance2.tex, append force
